...
import (
 ...
 func main() {
    ...
    if osNo != 3 {
        ...
        ntext := []byte(correct(string(text)))

        file.WriteString(string(ntext))
...
func ancorrect(n []string) []string {
	for x := 1; x < len(n); x++ {
		if len(n[x]) > 0 && (strings.Contains(strings.ToLower(string(n[x][0])), "o") || strings.Contains(strings.ToLower(string(n[x][0])), "u") || strings.Contains(strings.ToLower(string(n[x][0])), "i") || strings.Contains(strings.ToLower(string(n[x][0])), "e") || strings.Contains(strings.ToLower(string(n[x][0])), "a")) {
			if x-1 >= 0 && an(n[x-1]) && len(n[x-1]) == 1 {
				n[x-1] += "n"
			}
		}
	}
